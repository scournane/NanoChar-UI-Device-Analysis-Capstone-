...
tabview = customtkinter.CTkTabview(
    master=app, width=1000, height=800, anchor="w", corner_radius=10
)
...
def update_instrument_list():
    ...
    for idx, instrument in enumerate(instruments_list):
        label_inst = customtkinter.CTkLabel(
            scrollable_frame, text=f"{instrument['name']}", font=("Arial", 14, "bold")
        )
...
def show_panel():
    ...
    combobox_voltage_type = customtkinter.CTkComboBox(
        inst_window, values=["DC Voltage", "AC Voltage"]
    )
    ...
    combobox_sweep_type = customtkinter.CTkComboBox(
        inst_window, values=["Linear Sweep", "Log Sweep"]
    )
    ...
    button_add = customtkinter.CTkButton(
        inst_window, text="Add", command=add_instrument
    )
    button_add.grid(row=8, column=0, padx=10, pady=20)
    button_cancel = customtkinter.CTkButton(
        inst_window, text="Cancel", command=inst_window.destroy
    )
...
label = customtkinter.CTkLabel(
    master=tabview.tab("Settings"), text="Type", fg_color="transparent", pady=10
)
...
label = customtkinter.CTkLabel(
    master=tabview.tab("Settings"), text="Mode", fg_color="transparent"
)
label.grid(row=10, column=0)

combobox = customtkinter.CTkComboBox(
    master=tabview.tab("Settings"), values=["DC", "Pulse"], command=combobox_callback
)
combobox.set("DC")
combobox.grid(row=10, column=2)

# Changing Label================================
labelchg = customtkinter.CTkLabel(
    master=tabview.tab("Settings"), text="0", fg_color="transparent", pady=10
)
...
progressbar = customtkinter.CTkProgressBar(
    master=tabview.tab("Settings"), orientation="horizontal"
)
